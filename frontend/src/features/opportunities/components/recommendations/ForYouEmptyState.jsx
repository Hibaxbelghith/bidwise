...
const ForYouEmptyState = ({
  ...
  description = 'Add a few profile signals and BidWise AI can rank opportunities around your roles, preferences, experience, and profile context.',
}) => {
    ...
    <section className="rounded-md border border-neutral-200 bg-white p-6 text-center shadow-sm sm:p-8">
      ...
      {isUserAuthenticated ? (
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-2">
          {['Create Profile', 'Add Preferences', 'Upload Resume'].map((action) => (
            <span
              key={action}
      ...
      <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
        <Button asChild className="bg-blue-600 text-white hover:bg-blue-700">
          <Link to={isUserAuthenticated ? '/profile' : '/login'}>
            {isUserAuthenticated ? primaryHint?.ctaLabel || 'Review profile' : 'Create profile'}
